...
if isempty(argfile) %- INTERACTIVE MODE
   ...
   %--------------------------------------------------------------------
	  ...
	  [Ic,xCon]	    = deal(arg{:});
   	  xC 		    = xCon(Ic);
	  %try, save(Pcon,'xCon'); catch, fprintf('can not save xCon : check permissions\n'); end;
	  clear arg;
...
if isempty(argfile) %- INTERACTIVE MODE
   ...
   arg=mm_load_arg('Ci');
   arg.Ci.input.xX=SPM.xX; 
   arg.Ci.input.xCon=SPM.xCon; 
   arg 	= ui_arg(arg,'Ci');
   [Ic,SPM.xCon] 	= deal(arg{:});
   xC 		= SPM.xCon(Ic);
   save(fullfile(cwd{1},'SPM.mat'),'SPM'),;
   ...
   paramsAnal.temporalFilter 	= ui_arg(mm_load_arg('filter'),'filter');
	 ...
	 for sub=1:nbsub
	      [DIn Dsz]	=sf_strsplit(ImgName{sub},';');
		      for i=1:Dsz
			      pimg{i}=spm_get('Files',ImgDir{sub},str_clean(DIn{i}));
		      end
   	      Pimg{sub} 	= cat(1,pimg{:});
   	      Mask{sub}   	= fullfile(cwd{sub},'mask.img');
   	      load(fullfile(cwd{sub},'SPM.mat'),'VResMS');
